...
function CreatePurchaseAsset(props: any) {
  ...
  const getInvoiceNo = async (locationId:number) => {
    try {
      let invoiceurl =
        ...
        user?.id +
        `/${user?.companyInfo?.id}/${locationId}/purchaseAsset`;
  ...
  const onValuesChange = (column: any, allarray: any) => {
    try {
      ...
      if(column.ledger){
        purchaseAssetDetails(allarray?.ledger);
          ...
          (item: any, index: any) => {
            ...
            if (item && item.id !== null) {
              ...
              if (
                ...
                discountAmount = Number((price * quantity) * discountRate);
                total = price * quantity - discountAmount;
...
export default CreatePurchaseAsset;
