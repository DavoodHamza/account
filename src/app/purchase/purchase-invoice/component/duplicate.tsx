...
function PurchaseDuplicate(props: any) {
  ...
  const getInvoiceDetails = async () => {
    setIsLoading(true);
    try {
      let url = API.PURCHASE_SUPPLIER_LIST + id + "/purchase";
  ...
  const getInitialValues = (data: any) => {
    try {
      ...
      const initialValue = {
        ...
        outStanding: 0//Number(data?.invoiceDetails?.outstanding) || "",
      };
      setSubTotal(_subTotal);
      setTatalVat(_tatalVat);
      setOverolDiscount(_overollDiscount);
  ...
  return (
    <div>
      <PageHeader
        title={t("home_page.homepage.CreatePurchaseInvoice")}
        goBack={"/dashboard"}
...
export default PurchaseDuplicate;
