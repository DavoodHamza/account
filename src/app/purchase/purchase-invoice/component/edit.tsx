...
function Edit(props: any) {
        ...
        return {
          ...
          includevat:
            item.includeVat === null
              ? foundedProduct?.productDetails?.includevat === "0.00"
                ? false
                : true
              : item.includeVat,
          incomeTax: item.vat,
  ...
  const getInitialValues = (data: any) => {
    try {
      if (data?.invoiceItems[0]?.ledger) {
        props.getProduct(
          data?.invoiceItems[0]?.ledger === 12 ? "both" : "Service",data?.invoiceItems[0]?.seriesNo
        );
      }
      setIsStateTax(
      ...
      let _totalAmount = _subTotal + _tatalVat ;
      setTotalAmount(data?.invoiceDetails?.total);
  ...
  return (
    <div>
      <PageHeader
        title={t("home_page.homepage.UpdatePurchaseInvoice")}
        goBack={"/dashboard"}
        secondPathText={t("home_page.homepage.UpdatePurchaseInvoice")}
        secondPathLink={`/usr/purchace-invoice-form/update/${props.id}`}
