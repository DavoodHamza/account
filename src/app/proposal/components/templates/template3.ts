...
export const template3 = (data: any) => {
  const sidbarColor = data.primary_color || "#aeaeae";
  const fontfamily = `Arial, Helvetica, sans-serif`;
  const pageBackground =
    "https://bairuha-bucket.s3.ap-south-1.amazonaws.com/4940019.jpg";
  ...
  const listItem = (text: string) => {
    return `<li style="font-size: 20px; text-align: left;  margin-top: 10px; font-family:${fontfamily};">${text}</li>`;
  };
  const regex = /,\s*(?![^()]*\))/g;
        ...
        ${heddings("PROJECT PLAN")}
          ...
          <thead style={{ backgroundColor: "rgb(247, 247, 247)" }}>
            <tr>
              ...
              <th style="border: 1px solid black ;">Screens</th>
