...
function OtherPaymentScreen() {
  ...
  const handleOnEdit = (val: any) => {
    navigate(`/usr/other-payment/edit/${val?.id}`);
  };
  const columns = [
    {
      name: "id",
    ...
    },
    {
      ...
      alignment: "center",
      cellRender: ({ data }: any) =>
        data?.customer || data?.busname || data?.name,
    },
    {
      name: "paidTo",
  ...
  return (
    <>
      <PageHeader
        ...
        firstPathText={t("home_page.homepage.OtherPayment")}
        // buttonTxt={t("home_page.homepage.add")}
        // onSubmit={() => navigate("/usr/other-payment/create/0")}
      ...
      >
        <div>
          {canCreatePayments() && (
            <Button
              type="primary"
              onClick={() => navigate("/usr/other-payment/create/0")}
            >
              + {t("home_page.homepage.add")}{" "}
              {t("home_page.homepage.OtherPayment")}
            </Button>
          )}
        </div>
      </PageHeader>
