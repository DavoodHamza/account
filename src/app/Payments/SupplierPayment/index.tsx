...
function SupplierPaymentScreen() {
  ...
  const handleOnEdit = (val: any) => {
    navigate(`/usr/supplier-payment/edit/${val?.id}`);
  };
  const columns = [
    {
      name: "id",
  ...
  return (
    <>
      <PageHeader
        ...
        firstPathText={t("home_page.homepage.SupplierPayment")}
        // buttonTxt={t("home_page.homepage.add")}
        // onSubmit={() => navigate("/usr/supplier-payment/create/0")}
      ...
      >
        <div>
          {canCreatePayments() && (
            <Button
              type="primary"
              onClick={() => navigate("/usr/supplier-payment/create/0")}
            >
