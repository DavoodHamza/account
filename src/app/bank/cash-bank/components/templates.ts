    ...
    <body style="width: 100%;align-content: center;margin: auto;">
          ...
          <table style="margin-top: 25px !important;margin: auto;width: 100%;">
            <tbody> 
              ...
              <tr>
                ...
                <td>${moment(sale?.ldate).format("DD/MM/YYYY")}</td>
            ...
            <table style="margin-top: 25px !important;margin: auto;width: 100%;">
                <tbody style="text-align: center;">
                    <tr style="background: gray;color: white;text-align: center;font-size: 12px;">
                        ...
                        <td width="12%"><b>${user?.companyInfo?.tax === "gst" ? "GST%": "VAT%"}</b></td>
                        <td width="12%"><b>${user?.companyInfo?.tax === "gst" ? "GST": "VAT"}</b></td>
            ...
            <table style="margin-top: 25px !important;margin: auto;width: 100%;">
                <tbody style="text-align: center;">
                    ...
                    <tr style="font-size: 12px;">
                        ...
                        <td width="20%"><b>
                        ${user?.companyInfo?.countryInfo?.symbol || ""}${!isNaN(netTotal) ? parseFloat(netTotal?.toFixed(2)) : ""}
        ...
        user?.companyInfo?.countryInfo?.currency
            ...
            <table style="border-spacing: 0px;margin-top: 10px !important;margin: auto;width: 100%;">
               ...
               <br>
               <b>${user?.companyInfo?.tax === "gst" ? "GST":"VAT"}</b>:${parseFloat(vatTotal?.toFixed(2)) || 0}
               <br>
               <b>Grand Total</b>:${
                 user?.companyInfo?.countryInfo?.symbol || ""
               } ${parseFloat(
            ...
            <table style="margin-top: 25px !important;margin: auto;width: 100%;border: 1px solid black;">
                <tbody style="text-align: center;">
                    <tr style="background: gray;color: black;text-align: center;font-size: 12px;">
                        ...
                        <td width="12%"><b>PAID TO/FROM</b></td>
      ...
      `
          ...
          ${
            ...
            <td
              style="width:15%;border-bottom:1px solid gray;border-right:1px solid gray;border-left:1px solid gray;padding-left:120px">
    ...
    user?.companyInfo?.logo
          ...
          <tr>
              ...
              ${customer?.address ? `${customer?.address} <br> ` : ""}
                ${customer?.mobile ? `${customer?.mobile} <br> ` : ""}
                  ...
                  ${sale?.deladdress && `<b>Delivery Address</b><br>${sale?.deladdress}`}
      ...
      <tr>
         <td class="medium">
                ...
                <b>${user?.counterInfo === "gst" ? "Total GST" : "Total VAT"}:</b>  ${
                  vatTotal
                    ? `${user?.companyInfo?.countryInfo?.symbol} ${
      ...
      <table>
          <tr class="bordered">
              <th class="bordered">Bank Holder Name</th>
              <th class="bordered">PAID TO/FROM</th>
              <th class="bordered">Paid Method</th>
              <th class="bordered">Amount</th>
              <th class="bordered">Date</th>
              <th class="bordered">Payment Type</th>
          </tr>
         ${bankList
          .map(
          (bankInf: any) => `
          <tr class="bordered">
              <td class="bordered">${user.fullName || ""}</td>
              <td class="bordered">${bankInf?.bankInf?.laccount || ""}</td>
              <td class="bordered">${bankInf.paidmethod || ""}</td>
              <td class="bordered">${bankInf.amount}</td>
              <td class="bordered">${moment(bankInf.date).format("DD/MM/YYYY")}</td> 
              <td class="bordered"> ${bankInf.type}</td>  
              `
            )
            .join("")}       
          </tr> 
...
<td colspan="3"> <b  style="padding-top: 20px; padding-bottom: 20px;" >${
    pagetype ? pagetype : "Invoice"
              ...
              ${customer?.bus_name ? `${customer?.bus_name} <br> ` : ""}
                ${customer?.address ? `${customer?.address} <br> ` : ""}
                  ${customer?.mobile ? `${customer?.mobile} <br> ` : ""}
                    ...
                    ${customer?.email ? `${customer?.email} <br> ` : ""}
               ${user?.companyInfo?.tax === "gst" ? 
                `<b>GSTIN</b> ${customer?.vat_number}<br>` : `${customer?.vat_number} <br>`
               }
              ${
                sale?.deladdress
      ...
      <td style="height: 30px; " width="10%"><b>Item Code</b></td>
       ${
        user?.companyInfo?.tax === "gst"
          ? ` <td style="height: 30px; " width="10%"><b>HSN/SAC</b></td>`
          : null
      }
      <td width="10%"><b>Description</b></td>
      <td width="10%"><b>Price</b></td>
      ${path == "Service" ? "" : `<td width="10%"><b>Qtn</b></td>`}
      ${path == "Service" ? "" : `<td width="10%"><b>Unit</b></td>`}
      <td width="10%"><b>Discount <br>(Amt & %)</b></td>
      ${
        user?.companyInfo?.tax === "vat"
          ? `<td width="10%"><b>Vat <br>(Amt & %)</b></td>`
          : gstType
          ? `<td width="10%"><b>CGST <br>(Amt & %)</b></td>
          <td width="10%"><b>SGST <br>(Amt & %)</b></td>`
          : `<td width="10%"><b>GST <br>(Amt & %)</b></td>`
      }
     ...
     ${
          user?.companyInfo?.tax === "gst"
            ? `<td>${product?.product?.hsn_code ? product?.product?.hsn_code : ""}</td>`
            : ""
        }
        <td>
          ${product?.description ? product?.description : ""}
          ...
          <tbody style="text-align: center; background-color: #febb54;">
            <tr style="border: none;background: febb54;color: black;text-align: center;font-size: 12px;">
              ...
              <b>Taxable Value</b>: ${
               ...
               <br>
               <b>${user?.companyInfo?.tax === "gst" ?  "Total GST" : "Total VAT"}</b>:  ${
                 vatTotal
                   ? `${user?.companyInfo?.countryInfo?.symbol} ${
                       parseFloat(vatTotal.toFixed(2)) || ""
                     }`
  ...
  ${invoicearray.map(
    ...
    <tbody class="main-table main-table1" style="border-bottom: 1px solid black;">
        ...
        <tr class=${item?.length-1===index?"lineitem":"lineheightitem"} >
            ...
            <td>${product?.total ? `${user?.companyInfo?.countryInfo?.symbol}${parseFloat(
                       sale?.total || 0
                     ).toLocaleString()}` : ""}</td>
        </tr>`).join('')}
        </tbody>
    ...
    <td style="width: 40%; text-align: start; padding: 0;">
      <table class="page-break no-page-break">
        ...
        <tr>
          ...
          <td class="dataend">
            ${sale.total ? `${user?.companyInfo?.countryInfo?.symbol} ${parseFloat(
                       sale?.total || 0
                     ).toLocaleString()}` : ""}
          </td>
        </tr>
...
${invoicearray.map(
    ...
    <tbody class="main-table main-table1">
        ...
        <tr style="border-bottom:  1px solid black;">
            ...
            <td>${sale.total ? `${user?.companyInfo?.countryInfo?.symbol} ${parseFloat(
                       sale?.total || 0
                     ).toLocaleString()}` : ""}</td>
        </tr>
    </tbody>
