...
function BankTransfer() {
  ...
  const fetchBankList = async () => {
    try {
      setIsLoading(true);
      let bank_list_url = API.GET_BANK_LIST + adminid + '/' + user?.companyInfo?.id;
  ...
  // console.log("setBanksetBanksetBank",setBank)
  useEffect(() => {
    fetchBankList();
  }, []);
  ...
  const submitHandler = async (values: any) => {
    try {
      ...
      if (data.status) {
        notification.success({ message:"Success",description: "Transfer completed successfully" });
  ...
  return (
    <div>
      <PageHeader
        title={t("home_page.homepage.Bank_Transfer")}
        ...
        thirdPathText={t("home_page.homepage.Bank_Transfer")}
      />
      {isLoading ? (
        <LoadingBox />
      ) : (
            ...
            >
              <Row>
                ...
                <div className="formItem">
                  <label className="formLabel">{t("home_page.homepage.PaidFrom")}</label>
                  <Form.Item
                    name="paidfrom"
                  ...
                  <div className="formItem">
                  <label className="formLabel">{t("home_page.homepage.Paid_Method")}</label>
                  <Form.Item
                    name="paidmethod"
                    style={{ marginBottom: 10 }}
                    rules={[{ required: true }]}
                  >
                    <Select placeholder="choose one" size="large">
                      <Option value="cash">{t("home_page.homepage.Cash")}</Option>
                      <Option value="current">{t("home_page.homepage.Current")}</Option>
                      <Option value="cheque">{t("home_page.homepage.Cheque")}</Option>
                      <Option value="other">{t("home_page.homepage.Electronic")}</Option>
                      <Option value="card">{t("home_page.homepage.Credit/DebitCard")}</Option>
                      <Option value="loan">{t("home_page.homepage.PayPal")}</Option>
                      {/* <Option value="loan">PayPal</Option>   something wrong in value  */}
                    </Select>
                  </Form.Item>
                  </div>
                  <div className="formItem">
                  <label className="formLabel">{t("home_page.homepage.AmountTransferred")}</label>
                  <Form.Item
                    name="amount"
                    style={{ marginBottom: 10 }}
                    rules={[
                  ...
                  >
                    <Input type="number" placeholder={t("home_page.homepage.Amount")} size="large" />
                  </Form.Item>
                  </div>
                  <div className="formItem">
                  <label className="formLabel">{t("home_page.homepage.Description")}</label>
                  <Form.Item name="description" style={{ marginBottom: 10 }}>
                    <Input placeholder={t("home_page.homepage.Description")} size="large" />
                  </Form.Item>
                  </div>
                </Col>
                <Col md="6">
                <div className="formItem">
                  <label className="formLabel">{t("home_page.homepage.PaidTo")}</label>
                  <Form.Item
                    name="paidto"
                    style={{ marginBottom: 10 }}
                    rules={[{ required: true }]}
                  ...
                  >
                    <DatePicker
                      placeholder={t("home_page.homepage.Payment_Date")}
                      size="large"
                      style={{ width: "100%" }}
                  ...
                  // rules={[{ required: true }]}
                  >
                    <Input placeholder={t("home_page.homepage.Reference")} size="large" />
                  </Form.Item>
                        </div>
                        <br />
                  <Row>
