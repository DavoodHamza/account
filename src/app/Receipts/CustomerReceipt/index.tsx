...
function CustomerReceiptScreen() {
  ...
  const handleOnEdit = (val: any) => {
      navigate( `/usr/customer-receipt/edit/${val?.id}`);
  };
  const columns = [
    {
      name: "id",
  ...
  return (
    <>
      <PageHeader
        ...
        firstPathText={t("home_page.homepage.CustomerReceipt")}
        // buttonTxt={t("home_page.homepage.add")}
        // onSubmit={() => navigate("/usr/customer-receipt/create/0")}
      ...
      >
        <div>
          {canCreateReceipts() && (
            <Button
              type="primary"
              onClick={() => navigate("/usr/customer-receipt/create/0")}
            >
              + {t("home_page.homepage.add")}{' '}{t("home_page.homepage.CustomerReceipt")}
            </Button>
          )}
        </div>
      </PageHeader>
