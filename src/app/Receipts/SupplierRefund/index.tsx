...
function SupplierRefundScreen() {
  ...
  const handleOnEdit = (val: any) => {
      navigate( `/usr/supplier-refund/edit/${val?.id}`);
  };
  const columns = [
    {
      name: "id",
  ...
  return (
    <>
      <PageHeader
        ...
        firstPathText={t("home_page.homepage.SupplierRefund")}
        // buttonTxt={t("home_page.homepage.add")}
        // onSubmit={() => navigate("/usr/supplier-re fund/create/0")}
      ...
      >
        <div>
          {canCreateReceipts() && (
            <Button
              type="primary"
              onClick={() => navigate("/usr/supplier-refund/create/0")}
            >
              + {t("home_page.homepage.add")}{' '}{t("sidebar.title.supplier")}
            </Button>
          )}
        </div>
      </PageHeader>
...
export default SupplierRefundScreen;
