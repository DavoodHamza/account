...
function Create(props: any) {
  ...
  const onFinish = async (val: any) => {
    try {
      ...
      let column = val.columns.map((item: any) => {
        ...
        return {
          id: foundedProduct?.productId, // item.id,
      ...
      notification.error({
        ...
        description:
          "Failed to create proforma invoice!! Please try again later",
      });
    }
  };
  ...
  const onValuesChange = (column: any, allarray: any) => {
    try {
      ...
      if (allarray?.seriesNo) {
        props.getProduct("Stock", allarray?.seriesNo);
      }
      if (column.seriesNo) {
  ...
  return (
    <div>
      <PageHeader
        title={t("home_page.homepage.CreateProformaInvoice")}
        goBack={"/dashboard"}
        secondPathText={t("home_page.homepage.ProformaInvoice")}
        secondPathLink={"/usr/sales-proforma-invoice"}
        thirdPathText={t("home_page.homepage.CreateProformaInvoice")}
      ...
      {productCreate ? (
        <ProductAddModal
          ...
          productRefrush={() => props.getProduct("both")}
          type={"Nonstock"}
        />
      ) : null}
      {customerCreate ? (
