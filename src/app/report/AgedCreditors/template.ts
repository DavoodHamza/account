...
const agedCreditorsTemplate  = ({ user,payList,personalData}: any) => {
...
const totalPrice = payList.reduce((accumulator:any, currentValue:any) => {
    return Number(accumulator) + Number(currentValue.totalPrice);
  }, 0);
const currentMonthTotal = payList.reduce((accumulator:any, currentValue:any) => {
    return Number(accumulator) + Number(currentValue.currentMonthTotal);
  }, 0);
const lastTwoMonthsTotal = payList.reduce((accumulator:any, currentValue:any) => {
    return Number(accumulator) + Number(currentValue.lastTwoMonthsTotal);
  }, 0);
const lastThreeMonthsTotal = payList.reduce((accumulator:any, currentValue:any) => {
    return Number(accumulator) + Number(currentValue.lastThreeMonthsTotal);
  }, 0);
const lastFourMonthsTotal = payList.reduce((accumulator:any, currentValue:any) => {
    return Number(accumulator) + Number(currentValue.lastFourMonthsTotal);
  }, 0);
    ...
    >
      ...
      <body style="width: 100%; align-content: center; margin: auto">
        ...
        >
          ...
          "
        >
          <td colspan="3"><b>Aged Creditors Report</b></td>
        </tr>
...
>
        ...
        <div style="text-align: center; margin-top: 10px"> <span>${formattedCurrentDate}</span></div>
          ...
          `
                ...
                <td >
                <b>${currentMonthTotal}</b>
                </td>
                <td >
                <b>${lastTwoMonthsTotal}</b>
                </td>
                <td >
                <b>${lastThreeMonthsTotal}</b>
                </td>
                <td >
                <b>${lastFourMonthsTotal}</b>
                </td>
