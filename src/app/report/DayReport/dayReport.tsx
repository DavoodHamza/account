...
const DayReport = () => {
  ...
  const getDaySummary = async (
    ...
    try {
      ...
      let url =
        API.DAY_REPORT +
        `?sdate=${formattedDate}&ldate=${formattedEndDate}&companyid=${user?.companyInfo?.id}&page=${page}&limit=${limit}`;
    ...
    <>
      <Container>
        <DaySummaryTable
          ...
          handleDelete={(i?: any) => console.log("tesr")}
          loading={isLoading}
        />
      </Container>
      {emailModal && (
        <SendMailModal
          ...
          Attachment={`${user.companyInfo.bname}_trialBalance_${moment(
            new Date()
          ).format("DD-MM-YYYY")}`}
          defaultValue={{
            to: user.email,
