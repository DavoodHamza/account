...
function Payments({ balance }: any) {
  ...
  const LodeInvice = async (val: any) => {
    try {
      setIsLoading(true);
      let URL = API.PURCHASE_ID_BY_LIST + val + "/" + user?.id;
      const data: any = await GET(URL, null);
      if (data) {
  ...
  const onFinish = async (val: any) => {
    try {
      if (balance < val.amount) {
        ...
        let URL =
          type === "create"
            ? "purchaseinvoice/addSuppReceiptCash"
    ...
    <>
      ...
      {isLoading ? (
        ...
        <InvoiceTable
          ...
          columns={columns}
          saleId={saleId}
          tableHead={"Select Supplier Receipts and Invoices."}
