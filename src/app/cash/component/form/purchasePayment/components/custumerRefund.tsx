...
function CustumerRefund({ balance }: any) {
  ...
  const onFinish = async (val: any) => {
    try {
      ...
      notification.error({
        ...
        description: `Failed to ${
          type === "create" ? "create" : "update"
        } customer refund!! Please try again later`,
      });
      setIsBtLoading(false);
    }
  };
  ...
  const LoadRowData = (val: any) => {
    try {
      const amount = val?.reduce((acc: any, obj: any) => {
        return acc + Number(obj?.duplicateout);
      }, 0);
      setRows(val);
      setAmount(amount);
  ...
  const onValuesChange = (val: any, allarray: any) => {
    ...
    <>
      ...
      {isLoading ? (
        ...
        <InvoiceTable
          ...
          columns={columns}
          saleId={saleId}
          tableHead={"Select Customer Refund Receipts and Invoices."}
