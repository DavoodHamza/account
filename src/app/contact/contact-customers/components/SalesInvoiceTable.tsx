...
const SalesInvoiceTable = (props: any) => {
              ...
              content={
                <ViewPopover
                  onView={() => {
                    ...
                    else if (data.type === "Customer Receipt") {
                      // && bankid != null
                      navigate(
                  ...
                  OnEdit={() => {
                    if (data.type === "Journal") {
                      ...
                      navigate(
                        `/usr/purchace-debitnote-form/${data?.id}`
                      );
                    } else if(data.type === "stockassets"){
  ...
  const handleRowDoubleClick=(data:any)=>{
      if (data?.data?.type == "Journal") {
        navigate(`/usr/journal/details/${data?.data?.journalid}`);
      } else if (data?.data?.type == "Sales Invoice") {
        navigate(`/usr/sale-invoice-view/${data?.data?.id}`);
      } else if (data?.data.type == "Purchase Invoice") {
        navigate(`/usr/purchase-invoice-view/${data?.data.id}`);
      } else if (data?.data.type == "Credit Notes") {
        navigate(`/usr/salesCredit/view/${data?.data.id}`);
      } else if (data?.data.type == "Debit Notes") {
        navigate(`/usr/purchace-debitnote-view/${data?.data.id}`);
      }else if(data?.data.type === "stockassets"){
        navigate(`/usr/purchase-fore-assets`)
      }
      else if (data?.data.type === "Customer Receipt") {
        // && bankid != null
        navigate(
          `/usr/cashBank/customer-receipt/${data?.data?.id}/details`
        );
      }
      else if (data?.data.type === "Other Payment") {
        navigate(
          `/usr/cashBank/other-payment/${data?.data?.id}/details`
        );
      } else if (
        data?.data.type === "Customer Refund" &&
        data?.data.paidmethod !== "cash"
      ) {
        navigate(
          `/usr/cashBank/customer-refund/${data?.data?.id}/details`
        );
      } else if (data?.data.type === "Supplier Refund") {
        navigate(
          `/usr/cashBank/supplier-refund/${data?.data.id}/details`
        );
      } else if (data?.data.type === "Customer Reciept") {
        navigate(
          `/usr/cashBank/customer-receipt/${data?.data.id}/details`
        );
      } else if (data?.data.type === "Other Receipt") {
        navigate(
          `/usr/cashBank/other-receipt/${data?.data.id}/details`
        );
      }

      // cash
      else if (data?.data.paidmethod === "cash") {
        navigate(`/usr/cash/view/${data?.data.id}/${data?.data.ledger}`);
      } else {
        navigate(location.pathname);
      }
  ...
  return (
    <DataGrid
      ...
      showBorders={true}
      onExporting={(e) =>
        EXPORT(e, dataGridRef, "Customer details", customizeExportCell)
      }
        ...
        <TotalItem
          ...
          displayFormat={
            props?.openingBalance <= 0
              ? currency + " " + Math.abs((props?.openingBalance).toFixed(2))
              : "-"
          }
        />
        <TotalItem
